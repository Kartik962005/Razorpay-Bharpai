"""Command line: ``bharpai simulate``, ``bharpai report``, ``bharpai case``.
...
from bharpai.core import metrics as metrics_mod
from bharpai.core.audit import AuditLog, format_timeline
...
def _build_report(n, seed, scale, rows, collected, results) -> str:
    ...
    for winner, loser, title, note in (
        (
            ...
            "cases the naive policy recovered and Bharpai did not, usually because Bharpai declined "
...
def _writing_section(name: str, result) -> list[str]:
    ...
    from bharpai.core.models import Language
...
def sensitivity(
    ...
    from bharpai.sim.world import HOSTILE_ASSUMPTIONS, apply_overrides
...
def case(
    ...
    if case_id.startswith("live_"):
        ...
        console.print(
            ...
            "Run `bharpai live watch` against a test account first."
        )
        raise typer.Exit(1)
    ...
    if not path.exists():
        ...
        console.print(
            ...
            "  [bold]bharpai simulate[/bold]        (about 40 seconds, no keys needed)\n\n"
...
def serve(
    ...
    from bharpai.config import fingerprint, get_settings
    ...
    uvicorn.run("bharpai.api.app:app", host=host, port=port, log_level="warning")
...
def rules() -> None:
    ...
    from bharpai.core.policy import RULE_TEXT
...
from bharpai.live.cli import live as _live_app  # noqa: E402
