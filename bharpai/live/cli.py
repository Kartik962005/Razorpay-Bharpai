"""``bharpai live …`` — driving the agent against a real Razorpay test account."""
...
from bharpai.config import IST, fingerprint, get_settings
from bharpai.core.audit import AuditLog
from bharpai.core.models import CaseStatus
from bharpai.core.policy import PolicyEngine
from bharpai.live import state
from bharpai.live.poller import LivePoller
from bharpai.live.seed import next_steps, seed as seed_account
...
def _client():
    ...
    client.set_app_details({"title": "bharpai", "version": "0.1.0"})
    return client
...
def _reachable(url: str) -> str:
    ...
    try:
        ...
        return (
            ...
            "`bharpai live watch` needs no tunnel."
        )
...
def doctor() -> None:
    ...
    settings = get_settings()
    table = Table(title="bharpai live doctor", header_style="bold")
    ...
    table.add_row(
        ...
        if seeded
        else "none — run `bharpai live seed`",
    )
    console.print(table)
